...
def get_calculated_portfolio(username: str, db: Session = Depends(get_db)):
    ...
    positions_map = defaultdict(lambda: {'quantity': 0, 'total_cost': 0})
    # Use the account.trades relationship which is efficiently loaded by SQLAlchemy
    ...
    open_positions = []
    for ticker, data in positions_map.items():
        if data['quantity'] > 1e-9: # Use a very small number for float comparison
            open_positions.append({
                "ticker": ticker,
                "quantity": data['quantity'],
                "average_buy_price": data['total_cost'] / data['quantity']
...
def execute_trade(trade: TradeRequest, db: Session = Depends(get_db)):
    ...
    if trade.side == "BUY":
        ...
        # OPTIMIZATION: Calculate current shares from the existing relationship, not a separate API call.
        current_quantity = 0
        for t in account.trades:
            if t.ticker == trade.ticker:
                if t.side == TradeSide.BUY:
        ...
        if current_quantity < trade.quantity:
            raise HTTPException(status_code=400, detail=f"Not enough shares to sell. Current holdings: {current_quantity}")
