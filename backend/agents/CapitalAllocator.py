...
# from schemas.capital_schema import CapitalAllocatorResponse, Position, PortfolioState
# import json
# import math
# import os
...
# api_key = "AIzaSyDPge6khhejK0Y1V1DkP5nF47LQrAXXm78"
# # ---------- Capital Allocator ----------
...
#         """
#         # Just use the dict directly, or create a minimal PortfolioState schema
...
class CapitalAllocator:
    ...
    def _rule_allocate(self, signal: Dict[str, Any], portfolio: PortfolioState, fallback_reason: Optional[str] = None) -> CapitalAllocatorResponse:
        ...
        ticker = signal["ticker"]
        existing_mv = sum(p.market_value for p in portfolio.open_positions if p.ticker == ticker)
        headroom_ticker = max(0.0, portfolio.risk_limits.get("max_exposure_per_ticker_pct", 0.0) * equity - existing_mv)
        alloc_cap = min(alloc_cap, headroom_ticker)
    ...
    def _decision_from_cash(
        ...
        price = float(signal.get("current_price") or 0.0) or 1e-9
        qty = 0.0 if allocation_cash <= 0 else allocation_cash / price
        pct = 0.0 if equity <= 0 else min(allocation_cash / equity, portfolio.risk_limits.get("max_allocation_pct", 0.0))
