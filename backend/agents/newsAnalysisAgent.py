...
# GOOGLE_API_KEY2 = "AIzaSyBTBUkP6GxkBxN9p2gnHeCZsN6Ih1VxiHE"
# # Response model
# class NewsAgent():
...
from schemas.news_schema import NewsItem, NewsItemWithSentiment
from fastembed import TextEmbedding  # ✅ new import
...
class NewsAgent:
    def __init__(self):
        ...
        # ✅ Replace Google embeddings with fastembed
        self.local_embedder = TextEmbedding(
    ...
    def check_relevancy(self, symbol, article, threshold=0.5) -> bool:
        query = f"{symbol} stock news"
        article_text = f"{article.title} {article.summary}"

        # ✅ use fastembed instead of Google
        emb_query = list(self.local_embedder.embed([query]))[0]
        emb_article = list(self.local_embedder.embed([article_text]))[0]
